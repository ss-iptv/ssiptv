...
var Main = {
	...
	prefixURL : "http://filmix.net",
	janrURL : "http://filmix.net/",
	janrText : "Фильмы",
	search : false, 
	resMore: false,
	perPage : 21,
...
Main.keyDown = function() {
	...
	case tvKey.KEY_LEFT:
		if (Player.getState() == Player.PLAYING && this.mode == this.FULLSCREEN){
			...
			if (this.playlist == 0 && this.index>0 && CategoryElement_page[Main.Category_Settings] == '15') {
				if (this.index == 1) {
					this.smeh = Main.NewString(0, -1) ? (this.resMore?14:14) : 0;
					this.index = 1;
					Main.ActivString(this.smeh);
				}else{
					Main.ActivString(-1);
				}
			}else if (this.playlist == 0 && this.index>0 && CategoryElement_page[Main.Category_Settings] == '10') {
				if (this.index == 1) {
					this.smeh = Main.NewString(0, -1) ? (this.resMore?9:9) : 0;
					this.index = 1;
					Main.ActivString(this.smeh);
				}else{
					Main.ActivString(-1);
	...
	case tvKey.KEY_RIGHT:
		if (Player.getState() == Player.PLAYING && this.mode == this.FULLSCREEN) {
			...
			if (this.playlist == 0 && this.index<URLtoXML.ImgDickr.length && CategoryElement_page[Main.Category_Settings] == '15') {
				if (this.index == (this.resMore?15:15)) {
					this.index = 1;
					Main.NewString(0, 1);
					Main.ActivString(0);
			...
			}else if (this.playlist == 0 && this.index<URLtoXML.ImgDickr.length && CategoryElement_page[Main.Category_Settings] == '10') {
				if (this.index == (this.resMore?10:10)) {
					this.index = 1;
					Main.NewString(0, 1);
					Main.ActivString(0);
				}else if (this.index<URLtoXML.ImgDickr.length-1) {
	...
	case tvKey.KEY_UP:
		...
		if (this.playlist == 0 && !this.menu && CategoryElement_page[Main.Category_Settings] == '15') {
			this.smeh = this.resMore?-5:-5;
			if (this.index>0 && this.index<=(this.resMore?5:5)) {
				Main.NewString(this.resMore?10:10, -1);
			}
			Main.ActivString(this.smeh);
		}else if (this.playlist == 0 && !this.menu && CategoryElement_page[Main.Category_Settings] == '10') {
			this.smeh = this.resMore?-5:-5;
			if (this.index>0 && this.index<=(this.resMore?5:5)) {
				Main.NewString(this.resMore?5:5, -1);
			}
			Main.ActivString(this.smeh);
		}else if (this.playlist == 1) {
			Main.selectUpVideo();
	...
	case tvKey.KEY_DOWN:
		...
		if (this.playlist == 0 && !this.menu && CategoryElement_page[Main.Category_Settings] == '15') {
			this.smeh = this.resMore?5:5;
			if (this.index>(this.resMore?10:10) && this.index<=(this.resMore?15:15)) {
				Main.NewString((this.resMore?-10:-10), 1);
			}
			...
			this.smeh = this.resMore?5:5;
			if (this.index>(this.resMore?5:5) && this.index<=(this.resMore?10:10)) {
				Main.NewString((this.resMore?-5:-5), 1);
...
Main.NewJanr = function(janr) {
	...
	URLtoXML.Proceed(this.sURL);
		...
		if (CategoryElement_page[Main.Category_Settings] == '10') {
	    $("#bloc11").hide();
	    }
};
...
Main.contentlist = function() 
{
	...
	for (var key in category){
		if (category.hasOwnProperty(key))
		{
			...
			CategoryElement_page[value['chanel']] = value['page'];
		}
	}			
